...
**23,215 rows across 19 tables**, of which 17 are the tables your
statement is actually built on. The rest are small reference tables the others point at.

## Ninety seconds to your first query

```bash
sqlite3 data/PS-13.db < data/queries/starter_queries.sql
```

Or open `data/PS-13.db` in any SQLite client. Nothing to install, nothing to import.
...
In `PS-13.db` the money columns are declared `TEXT`. That is deliberate: SQLite's NUMERIC
...
```bash
python3 tools/validate_conformance.py data/PS-13.db     # or point it at your own export
```
